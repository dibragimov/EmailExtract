...
sacremoses_norm_punct = {'en': MosesPunctNormalizer(
    lang='en'), 'sv': MosesPunctNormalizer(lang='sv')}
sacremoses_tokenizers = {'en': MosesTokenizer(
    lang='en'), 'sv': MosesTokenizer(lang='sv')}
...
def Token(inp_fname, out_fname, lang='en',
    ...
    if not os.path.isfile(out_fname):
        ...
        curTokenizer = sacremoses_tokenizers.get(
            lang, MosesTokenizer(lang=lang))
        with open(out_fname, 'w') as outF:
            for line in sys.stdin:
                tok_norm_punct = curNormalizer.normalize(line)
                tok = curTokenizer.tokenize(
                    tok_norm_punct, return_str=True, escape=False)

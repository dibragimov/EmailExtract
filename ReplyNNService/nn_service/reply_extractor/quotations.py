...
from nn_service.reply_extractor.utils import (get_delimiter, html_tree_to_text,
                                        html_document_fromstring)
from nn_service.reply_extractor import html_quotations
...
RE_ORIGINAL_MESSAGE = re.compile(u'[\s]*[-]+[ ]*({})[ ]*[-]+'.format(
    u'|'.join((
        ...
        # Swedish
        u'Ursprungligt meddelande', 'Originalmeddelande', 'Svara meddelandet', 'svarsmeddelande',
    ))), re.I)

RE_FROM_COLON_OR_DATE_COLON = re.compile(u'((_+\r?\n)?[\s]*:?[*]?({})[\s]?:([^\n$]+\n){{1,2}}){{2,}}'.format(
    u'|'.join((
        # "From" in different languages.
        'From', 'Van', 'De', 'Von', 'Fra', u'Från', u'Lähettäjä',
...
def mark_message_lines(lines, use_nn=False):
    ...
    if not use_nn:
        ...
        while i < len(lines):
            if not lines[i].strip():
                ...
                # print('classifying this one:', lines[i])
                classes, probs = helper.classify(lines[i], 'sv')
                # print('classes, probs for {}'.format(lines[i]), classes, probs)
                if classes[0] == 'CONTENT':
                    markers[i] = 't'
                elif classes[0] == 'FWD_PART':   # FWD_PART
                    markers[i] = 's'
                else:
                    markers[i] = 't'
            i += 1
...
def extract_from_plain(msg_body, use_nn=False):
    ...
    markers = mark_message_lines(lines, use_nn)
